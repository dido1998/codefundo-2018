	...
	def trainGen(self, batch_size=8, is_Validation=False):
		...
		while True:
			...
			for index in range(i, end):
				...
				#print(self.train_dir + jpg_filename)
				png_filename = os.path.splitext(jpg_filename)[0] + '.jpg'
				#print(self.train_mask_dir + png_filename)
				mask_img_orig = cv2.imread(self.train_mask_dir + png_filename)
				
				binary_mask = get_street_mask(mask_img_orig)
				#print(binary_mask.shape)
				# another = np.zeros_like(binary_mask,dtype=int)
				# for i in range(512):
				# 	for j in range(512):
